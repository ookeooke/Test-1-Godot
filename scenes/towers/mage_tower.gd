...
# - Deals AOE (splash) damage to all enemies in splash_radius
# - Slower attack speed but hits multiple targets
# - Frost path can slow enemies
# ============================================

# TOWER IDENTITY
@export var tower_id: String = "mage"
...
var stat_range: Stat
var stat_splash_radius: Stat  # NEW: AOE radius
...
# UPGRADE SYSTEM
var tower_level = 1
var upgrade_path = ""  # "" = not chosen, "inferno" = damage, "frost" = slow
...
var mage_weapon: Node2D  # Visual weapon node
...
func _initialize_stats():
	...
	if level_1_stats.is_empty():
		push_error("[MageTower] CRITICAL: Failed to load stats from TowerData!")
	...
	print("[MageTower] Stats initialized - DMG: %.1f, AS: %.1f, Range: %.1f, Splash: %.1f" %
...
func _ready():
	...
	# Get mage weapon reference
	if has_node("Mage/Weapon"):
	...
	print("[MageTower] Ready - Build cost: %dg, Range: %.1f" % [build_cost, range_radius])
...
func _update_range_indicator():
	...
	range_indicator.color = Color(0.3, 0.3, 1.0, 0.2)  # Blue tint for mage
	range_indicator.visible = is_selected

func _process(_delta):
	# Update weapon rotation toward target
	if current_target and is_instance_valid(current_target) and mage_weapon:
...
func _on_enemy_entered_range(body):
	...
	# Check if it's an enemy (has required methods/properties)
	...
	if not body in enemies_in_range:
		...
		# If we don't have a target, acquire this one
...
func _get_last_enemy():
	"""Get enemy closest to path start"""
	var best = null
...
func _get_enemy_progress(enemy) -> float:
	"""Get enemy progress along path (0.0 to 1.0)"""
...
func _record_target_change_time():
	"""Record time of target change for stickiness"""
...
func shoot_at(target):
	"""Shoot magical projectile at target with AOE damage"""
	if projectile_scene == null:
		print("ERROR: No projectile scene assigned to Mage Tower!")
		return
	...
	# Add frost slow effect if on frost path (only at levels 4-5)
	...
	print("[MageTower] Fired magic projectile - DMG: %.1f, Splash: %.1f" % [damage, splash_radius])
...
func _on_mouse_entered():
	...
	pass  # Add hover visual if desired

func _on_mouse_exited():
	"""Mouse exited tower area"""
	pass  # Remove hover visual if desired
...
func upgrade_tower():
	...
	print("🔧 [MageTower] upgrade_tower() called - Current Level: %d" % tower_level)

	# Check if at Level 3 (needs path choice) or at max level
	if tower_level == MAX_LEVEL_BEFORE_CHOICE and upgrade_path == "":
		push_warning("[MageTower] Cannot upgrade past level 3 - must choose path!")
		return false

	if tower_level >= MAX_LEVEL:
		push_warning("[MageTower] Tower already at MAX LEVEL!")
		return false
	...
	print("🔧 [MageTower] Level changed: %d → %d (Cost: %dg, Total: %dg)" % [old_level, tower_level, upgrade_cost, build_cost])
	...
	if target_stats.is_empty():
		push_error("[MageTower] Failed to load upgrade stats from TowerData for level %d" % tower_level)
		return false
	...
	print("✅ [MageTower] Upgraded to Level %d: DMG=%.1f, AS=%.1f, Range=%.1f, Splash=%.1f, DPS=%.1f" % [tower_level, damage, attack_speed, range_radius, splash_radius, damage * attack_speed])
...
func choose_inferno_path():
	"""Choose inferno specialization path (Level 3 → 4 Inferno) - Maximum AOE damage"""
	if tower_level != MAX_LEVEL_BEFORE_CHOICE:
		push_warning("[MageTower] Can only choose path at level 3!")
		return false

	if upgrade_path != "":
		push_warning("[MageTower] Path already chosen: %s" % upgrade_path)
		return false
	...
	# Load inferno path stats from TowerData
	var level_1_stats = TowerData.get_tower_stats(tower_id, 1)
	var target_stats = TowerData.get_tower_stats(tower_id, 4, "inferno_path")

	if target_stats.is_empty():
		push_error("[MageTower] Failed to load inferno path stats from TowerData")
		return false
	...
	# Apply modifiers
	var mod_source = "upgrade_path_inferno"
	if damage_delta != 0:
		stat_damage.add_modifier(StatModifier.create_flat(damage_delta, mod_source, "Inferno Path Level 4"))
	if attack_speed_delta != 0:
		stat_attack_speed.add_modifier(StatModifier.create_flat(attack_speed_delta, mod_source, "Inferno Path Level 4"))
	if range_delta != 0:
		stat_range.add_modifier(StatModifier.create_flat(range_delta, mod_source, "Inferno Path Level 4"))
	if splash_delta != 0:
		stat_splash_radius.add_modifier(StatModifier.create_flat(splash_delta, mod_source, "Inferno Path Level 4"))

	print("[MageTower] INFERNO PATH chosen: DMG=%.1f, AS=%.1f, Splash=%.1f, DPS=%.1f - Maximum magical devastation!" % [damage, attack_speed, splash_radius, damage * attack_speed])
	...
	# Track upgrade in BalanceTracker
	if BalanceTracker:
		BalanceTracker.record_tower_upgrade(self, tower_level, path_cost, "inferno")

	return true

func choose_frost_path():
	"""Choose frost specialization path (Level 3 → 4 Frost) - Enemy slowing and control"""
	if tower_level != MAX_LEVEL_BEFORE_CHOICE:
		push_warning("[MageTower] Can only choose path at level 3!")
		return false
	...
	# Load frost path stats from TowerData
	var level_1_stats = TowerData.get_tower_stats(tower_id, 1)
	var target_stats = TowerData.get_tower_stats(tower_id, 4, "frost_path")

	if target_stats.is_empty():
		push_error("[MageTower] Failed to load frost path stats from TowerData")
		return false
	...
	# Apply modifiers
	var mod_source = "upgrade_path_frost"
	if damage_delta != 0:
		stat_damage.add_modifier(StatModifier.create_flat(damage_delta, mod_source, "Frost Path Level 4"))
	if attack_speed_delta != 0:
		stat_attack_speed.add_modifier(StatModifier.create_flat(attack_speed_delta, mod_source, "Frost Path Level 4"))
	if range_delta != 0:
		stat_range.add_modifier(StatModifier.create_flat(range_delta, mod_source, "Frost Path Level 4"))
	if splash_delta != 0:
		stat_splash_radius.add_modifier(StatModifier.create_flat(splash_delta, mod_source, "Frost Path Level 4"))

	print("[MageTower] FROST PATH chosen: DMG=%.1f, AS=%.1f, Splash=%.1f, Slow=%.0f%% for %.1fs - Crowd control master!" % [damage, attack_speed, splash_radius, target_stats.get("slow_amount", 0.5) * 100, target_stats.get("slow_duration", 2.0)])
	...
	# Track upgrade in BalanceTracker
	if BalanceTracker:
		BalanceTracker.record_tower_upgrade(self, tower_level, path_cost, "frost")
	...
	for child in detection_range.get_children():
		if child is CollisionShape2D:
			...
			if shape and shape is CircleShape2D:
				...
				print("[MageTower] Detection range updated to: %d" % range_radius)
			else:
				push_error("[MageTower] Failed to update detection range - shape is invalid or not CircleShape2D")
			break
...
func _revalidate_enemies_in_range():
	...
	for enemy in enemies_in_range:
		...
		if distance > range_radius:
			enemies_to_remove.append(enemy)
			print("[MageTower] Enemy removed from range after range update (distance: %d > %d)" % [distance, range_radius])
...
func get_upgrade_cost() -> int:
	...
	# Convert upgrade_path ("inferno"/"frost") to TowerData format ("inferno_path"/"frost_path")
	var path_param = ""
	if upgrade_path != "":
	...
	print("⚠️ [Mage] WARNING: No cost_to_next found for level %d, path '%s'" % [tower_level, upgrade_path])
	return 0
...
func get_upgrade_stats(preview_path: String = "") -> Dictionary:
	...
	Args:
		preview_path: For Level 3, specify "inferno" or "frost" to preview that path choice
	"""
...
func draw_range_circle():
	...
	# Set purple/blue color for mage tower
	range_indicator.color = Color(0.3, 0.3, 1.0, 0.3)  # Purple-blue, 30% opacity
